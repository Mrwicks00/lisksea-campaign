...
export const NFTCard = ({ tokenId }: NFTCardProps) => {
  ...
  const { data: priceFeedContract } = useDeployedContractInfo("PriceFeed" as any);
  ...
  // Get listing info - FIX: Use type assertion on the entire hook call
  const { data: listingRaw, refetch: refetchListing } = useScaffoldContractRead({
    contractName: "NFTMarketplace" as any,
    functionName: "getListing",
    args: [BigInt(tokenId)],
  } as any);

  // Type assertion for listing data
  const listing = listingRaw as unknown as NFTListing | undefined;

  // Get marketplace contract info
  const { data: marketplaceContract } = useDeployedContractInfo("NFTMarketplace" as any);
  ...
  // Contract writes - FIX: Use type assertion on the entire configuration object
  ...
  const { writeAsync: buyItem } = useScaffoldContractWrite({
    ...
    args: [BigInt(tokenId)],
    value: listing?.isActive ? listing.price : undefined,
  } as any);
